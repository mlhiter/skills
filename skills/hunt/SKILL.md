...
## First-Principles Root-Cause Pass

For non-trivial failures, recurring bugs, production incidents, architectural regressions, or any fix that initially looks like a one-line patch, force the diagnosis back to first principles before editing:

1. Name the invariant that should have held.
2. Identify the source of truth and the boundary that owns it.
3. Trace the causal chain from entrypoint to symptom, including config, cache, retry, time, auth, tenant, network, and deploy boundaries when relevant.
4. List the assumptions that came from analogy ("this looks like the old bug", "probably cache", "just missing validation") and mark each as proven or unproven.
5. Ask whether the proposed fix removes the failure mechanism or only masks the reported symptom.

If this pass exposes a lower-layer mechanism than the first hypothesis, discard the symptom-level fix and continue diagnosis from the lower layer. A first-principles pass does not replace runtime evidence; it decides where to collect that evidence.
