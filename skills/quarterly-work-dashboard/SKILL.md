...
- 视觉结构：采用克制的执行层分析台风格，首屏最多包含核心 KPI、一条来源/可信度信号栏、一条价值路径和 2-3 张总览图；不要在首屏堆叠多组同质卡片。
...
- 图表层级：总览图只解释核心指标，例如交付构成、环比变化、成果结构；主题投入、工作类型、语言结构等细节图放在 GitHub 详情或审计层。
...
- 首屏是否足够克制：核心 KPI 下方是否只有信号栏、质量 rail、价值 rail 和 2-3 张关键图，没有连续卡片墙？
