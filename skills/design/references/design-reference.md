...
`VoltAgent/awesome-design-md` maintains a curated catalog of DESIGN.md files extracted from real-world brand sites. Running `npx getdesign@latest add <brand>` drops the file into the project root, giving the agent concrete token values to decompose rather than reasoning from memory.

**Usage rule:** never auto-run the command. Offer it as an option during Reference-App Direction Mode or direction lock, run it only with explicit user approval, and treat the result as seed decomposition material, not a finished direction.

**Use it when:**
- The user asks for a new project page, new page, or new component and has not locked a visual direction.
- The user asks for broad style improvement: "make it better", "more premium", "less generic", "optimize the styling".
- The user names a brand or product reference, for example Linear, Vercel, Claude, Notion, Stripe, Apple, or "copy X's style".

**Do not use it when:**
- The task is a narrow visual quick fix with a concrete defect.
- The current product already has source tokens, design docs, screenshots, or a mature visual system that should be preserved.
- The requested reference conflicts with the user's product category, accessibility requirements, or performance constraints.

**Brands in the catalog** (recognize these when a user names a reference):

| Category | Brands |
|---|---|
| AI & LLM | Claude, Cohere, ElevenLabs, Minimax, Mistral, Ollama, OpenCode AI, Replicate, RunwayML, Together AI, VoltAgent, xAI |
...
| Media & Consumer | Apple, HP, IBM, NVIDIA, Pinterest, PlayStation, SpaceX, Spotify, The Verge, Uber, Vodafone, WIRED |
| Automotive | BMW, BMW M, Bugatti, Ferrari, Lamborghini, Tesla |
...
**Brand transfer checklist:** after reading a preset, extract these before coding:
- Color roles: canvas, surface, text, muted text, border, accent, danger/success if needed.
- Typography: display face, body face, mono face, size scale, weight scale, letter-spacing rules.
- Geometry: radius scale, button shape, spacing rhythm, grid width, card or cardless section treatment.
- Depth: background-color steps, shadows, borders, image treatment, and whether surfaces feel flat, layered, or editorial.
- Interaction: one motion signature and exact implementation constraint, such as `transform`/`opacity` only, press scale, staggered reveal, or icon cross-fade.
- Negative space: which brand traits to avoid because they would hurt this product's content, density, or accessibility.

**Fallback:** if `npx getdesign@latest add <brand>` fails or the brand is not in the catalog, do not invent exact tokens from memory. Ask for a screenshot, read source files if available, or propose adjacent reference apps with explicit differences. If none are available, lock a fresh direction from product category and user context.
...
For well-known brands catalogued in `awesome-design-md`: ask the user whether to pull the curated preset via `npx getdesign@latest add <brand>`. If they approve, run it, read the generated `DESIGN.md` at project root, then decompose the reference into the Brand transfer checklist above rather than from memory. The preset is a starting point, not a finished direction: the user still names or accepts the aesthetic precisely, the current product's constraints still apply, and the reflex-font blocklist and absolute bans still win on any conflict.
