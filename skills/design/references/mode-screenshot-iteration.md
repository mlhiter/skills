...
Loaded from `design` when the user supplies a screenshot of a rendered surface and wants it improved against that evidence.
...
8. Verify the matrix in a browser, native app, screenshot tool, or rendered artifact, then re-check every `preserve` boundary. A final-state screenshot does not prove copy feedback, touch behavior, first paint, or transition stability. If rendering is available, finish the loop yourself and report the checked states, then ask the user to verify the same real screen. If the host cannot render, say that explicitly and hand off the shortest exact view the user should check.
...
**Redesign priority order** (when reworking an existing UI rather than building from scratch): font replacement → color cleanup → hover/active states → layout and whitespace → replace generic components → add loading/empty/error states → typographic polish. This order maximizes visual lift while minimizing the blast radius of each pass. Full rules, common traps, and absolute CSS bans all live in `design-reference.md`.
