...
Loaded from `design` when the ask is a bounded visual correction to an existing screen, not a new build.
...
5. If the fix touches three or more components, changes product behavior, or reveals a direction problem, stop and switch to `mode-screenshot-iteration.md` or the Lock the Direction First section of `SKILL.md`.

**Spacing unification rule.** A spacing or sizing value tuned three times that still looks off is structural, not numeric: collapse the N independent values into one shared named token (`Spacing.s4`, `--gap-content`), with outer container padding defaulting to the inner element gap. Spacing-as-a-system details live in `design-reference.md`.
