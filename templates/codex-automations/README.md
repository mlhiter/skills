...
| [`local-project-news-weekdays`](local-project-news-weekdays/automation.toml) | Weekdays at 10:00 | News, releases, and security updates related to local projects. |
...
Or ask it to review all four first:
...
Review the prompt and data sources before enabling a template. The work-review and local-project templates inspect local repositories in read-only mode. The news templates access public network sources and may depend on source availability.
...
模板刻意省略了 automation ID、目标任务 ID、项目 ID、本机路径和时间戳。调度默认按 Codex app 的本地时区执行。启用前应检查提示词和数据来源；工作回顾与本地项目新闻模板只读检查本地仓库，新闻类模板会访问公开网络来源。
